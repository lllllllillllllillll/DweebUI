...
export const Apps = async function(req,res){
    ...
    // Set the page and template to defaults if they are not set
    ...
    let pages =`<li class="page-item"><a class="page-link" href="/apps/view/page/1/">1</a></li>
    ...
    let prev = '/apps/view/page/' + (page - 1) + '/';
    let next = '/apps/view/page/' + (page + 1) + '/';
    if (page == 1) { prev = '/apps/view/page/' + (page) + '/'; }
    ...
    for (let i = 0; i < json_files.length; i++) {
      if (json_files[i] != 'default.json') {
        ...
        let link = `<li><a class="dropdown-item" href="/apps/view/template/${filename}">${filename}</a></li>`
        json_templates += link;
      }
    }
    ...
    if (template == 'compose') {
      ...
      for (let i = list_start; i < list_end && i < templates_global.length; i++) {
          ...
          for (let j = 0; j < templates[i].categories.length; j++) {
            categories += CategoryColor(templates[i].categories[j]);
          }
...
export const AppsAction = async function (req, res) {
    ...
    if (action == 'upload') {
      ...
      upload.array('files', 10)(req, res, () => {
        let files = readdirSync('appdata/tmp/');
    
        for (let i = 0; i < files.length; i++) {
          // Zip files
          if (files[i].endsWith('.zip')) {
            let zip = new AdmZip(`appdata/tmp/${files[i]}`);
            zip.extractAllTo('appdata/compose', true);
            unlinkSync(`appdata/tmp/${files[i]}`);

          }    // JSON files
          else if (files[i].endsWith('.json')) {
            if (existsSync(`appdata/templates/${files[i]}`)) {
              unlinkSync(`appdata/tmp/${files[i]}`);
              res.redirect('/apps');
              return;
            }
            renameSync(`appdata/tmp/${files[i]}`, `appdata/templates/${files[i]}`);
          } 
          else if (files[i].endsWith('.yml')) {
            let compose = readFileSync(`appdata/tmp/${files[i]}`, 'utf8');
            let compose_data = parse(compose);
            let service_name = Object.keys(compose_data.services);
            if (existsSync(`appdata/compose/${service_name}`)) {
              unlinkSync(`appdata/tmp/${files[i]}`);
              res.redirect('/apps');
              return;
            }
            mkdirSync(`appdata/compose/${service_name}`);
            renameSync(`appdata/tmp/${files[i]}`, `appdata/compose/${service_name}/compose.yaml`);
            ...
            mkdirSync(`appdata/compose/${service_name}`);
            renameSync(`appdata/tmp/${files[i]}`, `templates/compose/${service_name}/compose.yaml`);
          } else {
    ...
    // Remove template
    if (action == 'remove') {
      unlinkSync(`appdata/templates/${id}.json`);
      res.redirect('/apps');
      return;
    }
